...
def index(request):
    ...
    return render(request,'index.html',{'user_profile': user_profile, 'Upload_post': feed_lists, 'suggestions':suggestions[:4]})
    ...
    if request.method == 'POST':
        ...
        if password == password2:
            if User.objects.filter(email=email).exists():
                ...
                #Create a Profile objects for the new User
                user_model = User.objects.get(username=username)
                new_profile = Profile.objects.create(user = user_model, id_user = user_model.id)
                new_profile.save()
                return redirect('settings')
        else:
            messages.info(request, "Confirm Password is not Matching")
            return redirect('signup')
